...
static std::atomic<double> level3time(0);
...
void distributed_strassen(GlobalAddress<int32_t> a, GlobalAddress<int32_t> b, GlobalAddress<int32_t> c, GlobalAddress<int32_t> tim, size_t m0, u_int32_t level, u_int32_t offset)
{
    ...
    if (level == 3) {
        int level3time = walltime() - matmul_start;
        delegate::write(tim, level3time);
    }
...
// distributed strassen multiplication
void single_distributed_strassen(GlobalAddress<int32_t> a, GlobalAddress<int32_t> b, GlobalAddress<int32_t> c, GlobalAddress<int32_t> tim, size_t m0, u_int32_t level, u_int32_t offset)
{
    std::cout << "level " << level << " core " << mycore()  << " locale " << mylocale() << " thread offset " << offset << ": single_distributed_strassen start" << std::endl;
    double distributed_start = walltime();
    size_t m = m0 / 2;
    // Submatrix indices
    size_t tl_row_start = 0, tl_col_start = 0;
    ...
    {
        forall<SyncMode::Async, &level1_gce>(params1_addr, 1, [=](int64_t i, Params &params1){
            single_distributed_strassen(aa1, bb1, m1, tim, m, level + 1, offset*7+1);
        });

        forall<SyncMode::Async, &level1_gce>(params2_addr, 1, [=](int64_t i, Params &params2){
            single_distributed_strassen(aa2, bb2, m2, tim, m, level + 1, offset*7+2);
        });

        forall<SyncMode::Async, &level1_gce>(params3_addr, 1, [=](int64_t i, Params &params3){
            single_distributed_strassen(aa3, bb3, m3, tim, m, level + 1, offset*7+3);
        });

        forall<SyncMode::Async, &level1_gce>(params4_addr, 1, [=](int64_t i, Params &params4){
            single_distributed_strassen(aa4, bb4, m4, tim, m, level + 1, offset*7+4);
        });

        forall<SyncMode::Async, &level1_gce>(params5_addr, 1, [=](int64_t i, Params &params5){
            single_distributed_strassen(aa5, bb5, m5, tim, m, level + 1, offset*7+5);
        });

        forall<SyncMode::Async, &level1_gce>(params6_addr, 1, [=](int64_t i, Params &params6){
            single_distributed_strassen(aa6, bb6, m6, tim, m, level + 1, offset*7+6);
        });

        forall<SyncMode::Async, &level1_gce>(params7_addr, 1, [=](int64_t i, Params &params7){
            single_distributed_strassen(aa7, bb7, m7, tim, m, level + 1, offset*7+7);
        });
        level1_gce.wait();
    }
    else if (level == 2){
    ...
    run([]
        {
            ...
            auto tim = global_alloc<int32_t>(1);

            double start = walltime();
            if (cores() == 16) {
                spawn<&gce>([=]
                        { single_distributed_strassen(A, B, C, tim, MATRIX_SIZE, 1, 0); });

            } else {
                spawn<&gce>([=]
                        { distributed_strassen(A, B, C, tim, MATRIX_SIZE, 1, 0); });
            }
            ...
            std::ofstream myFile;
            int gemm_time = delegate::read(tim);
            int core_num = cores();
            char logname[256] = "/mnt/ssd/guest/DRust_home/logs/gemm_grappa_";
