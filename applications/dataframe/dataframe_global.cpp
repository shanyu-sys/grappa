...
public:
    ...
    uint8_t* get(size_t index) {
        ...
        uint8_t* result = new uint8_t[elementSize];
        std::memcpy(result, buffer + index * elementSize, elementSize);
        return result;
    }
    ...
    void set(size_t index, const void *data) {
        ...
        std::memcpy(buffer + index * elementSize, data, elementSize);
    }
};
...
private:
    size_t elementSize;
    size_t _num_chunks;
    GlobalAddress<Chunk> chunks;
    // std::vector<std::shared_ptr<Chunk>> chunks;
    // std::vector<GlobalAddress<Chunk>> global_chunks;

public:
    ChunkedArray(size_t elementSize, size_t capacity) : elementSize(elementSize) {
        _num_chunks = (capacity * elementSize + CHUNK_SIZE - 1) / CHUNK_SIZE;
        chunks = global_alloc<Chunk>(_num_chunks);
        printf("num_chunks: %ld\n", _num_chunks);
        std::cout<<capacity - (_num_chunks - 1) * (CHUNK_SIZE / elementSize)<<std::endl;
        forall(chunks, _num_chunks, [=](int64_t i, Chunk &a)
            {
                std::cout << "init chunk" << std::endl;
                a.elementSize = elementSize; 
                if(i == _num_chunks - 1) {
                    // a.set_length(capacity - (_num_chunks - 1) * (CHUNK_SIZE / elementSize));
                    a.len = capacity - (_num_chunks - 1) * (CHUNK_SIZE / elementSize);
                } else {
                    // a.set_length(CHUNK_SIZE / elementSize);
                    a.len = CHUNK_SIZE / elementSize;
                }
            });
        // for (int i = 0; i < _num_chunks; i++) {
        //     Chunk chunk = delegate::read(chunks + i);
        //     chunk.elementSize = elementSize;
        //     if(i == _num_chunks - 1) {
        //         // a.set_length(capacity - (_num_chunks - 1) * (CHUNK_SIZE / elementSize));
        //         chunk.len = capacity - (_num_chunks - 1) * (CHUNK_SIZE / elementSize);
        //     } else {
        //         // a.set_length(CHUNK_SIZE / elementSize);
        //         chunk.len = CHUNK_SIZE / elementSize;
        //     }
        //     delegate::write(chunks + i, chunk);
        // }
    ...
    uint8_t * get(size_t index) {
        ...
        // Could optimize this by using delegate::call
        Chunk chunk = delegate::read(chunks + chunkIndex);
        uint8_t* result = chunk.get(chunkOffset);
        return result;
    }
    ...
    void put(size_t index, const void *data) {
        ...
        // Could optimize this by using delegate::call
        Chunk chunk = delegate::read(chunks + chunkIndex);
        chunk.set(chunkOffset, data);
        delegate::write(chunks + chunkIndex, chunk);
    }
};
...
Series read_series(const std::string& file_name, std::string datatype, size_t series_id, size_t line_count) {
    ...
    ChunkedArray data(element_size, line_count);
    ...
    for (int i = 0; i < line_count; i++) {
        ...
        std::cout << "value: " << *((int32_t*) value) << std::endl;
        // data.append(value);
...
void test_groupby_agg(std::string file_name, int line_count) {
    ...
    group_by_column.push_back(series_value1);
    group_by_column.push_back(series_value2);
